...
def natural_cubic_interpolation(x: np.ndarray, y: np.ndarray) -> list:
    ...
    for i in range(n - 2):
        ...
        matrix[4 * i][4 * i] = 1
        matrix[4 * i][4 * i + 1] = x[i]
        matrix[4 * i][4 * i + 2] = x[i] ** 2
        matrix[4 * i][4 * i + 3] = x[i] ** 3

        # 2 Zeile erster Matrix
        matrix[4 * i + 1][4 * i] = 1
        matrix[4 * i + 1][4 * i + 1] = x[i + 1]
        matrix[4 * i + 1][4 * i + 2] = x[i + 1] ** 2
        matrix[4 * i + 1][4 * i + 3] = x[i + 1] ** 3
        ...
        matrix[4 * i + 2][4 * i + 2] = 2 * x[i + 1]
        matrix[4 * i + 2][4 * i + 3] = 3 * x[i + 1] ** 2

        # 4 Zeile erster Matrix
        matrix[4 * i + 3][4 * i] = 0
        matrix[4 * i + 3][4 * i + 1] = 0
        matrix[4 * i + 3][4 * i + 2] = 2
        matrix[4 * i + 3][4 * i + 3] = 6 * x[i + 1]

        # erste Zeile zweiter Matrix
        matrix[4 * i + 2][4 * i + 4] = 0
        matrix[4 * i + 2][4 * i + 5] = -1
        matrix[4 * i + 2][4 * i + 6] = -2 * x[i + 1]
        matrix[4 * i + 2][4 * i + 7] = -3 * x[i + 1] ** 2

        # zweite Zeile zweiter Matrix
        matrix[4 * i + 3][4 * i + 4] = 0
        matrix[4 * i + 3][4 * i + 5] = 0
        matrix[4 * i + 3][4 * i + 6] = -2
        matrix[4 * i + 3][4 * i + 7] = -6 * x[i + 1]
...
def periodic_cubic_interpolation(x: np.ndarray, y: np.ndarray) -> list:
    ...
    # periodische randbedingungen zeilen zeilen
    matrix[n - 2][0] = 0
    matrix[n - 2][1] = 1
    matrix[n - 2][2] = 2 * x[0]
    matrix[n - 2][3] = 3 * x[0] ** 2

    matrix[n - 2][n-4] = 0
    matrix[n - 2][n-3] = -1
    matrix[n - 2][n-2] = -2 * x[x.size-1]
    matrix[n - 2][n-1] = -3 * x[x.size-1] ** 2
